...
function memoryAdd(targetSlot, operation = "+") {
  const rawText = (levMode ? displaylv : display).textContent.trim(); // Променено за div
  const cleanText = rawText.replace(/\s/g, '').replace(',', '.');
  const value = parseFloat(cleanText);
  if (isNaN(value)) {
    console.warn("❗ Невалидна стойност: ", cleanText);
    return;
  }
  if (targetSlot === 4) {
    // Специално поведение за слот 4
    // handleSpecialMemorySlot(value, operation);
    return;
  }
  ...
  switch (operation) {
    case "+":
      ...
      console.log(`🧠 M+ към Mem[${targetSlot}] → +${value} = [${Mem}]`);
      updateMemoryStatusDisplay(targetSlot, true); // Променя фона на светлосин
      setTimeout(() => {
          updateMemoryStatusDisplay(targetSlot, false); // Връща оригиналния фон след 3 секунди
      }, 300);
      break;
    case "-":
      Mem[targetSlot] -= value;
      console.log(`🧠 M− от Mem[${targetSlot}] → −${value} = [${Mem}]`);
      updateMemoryStatusDisplay(targetSlot, true); // Променя фона на светлосин
      setTimeout(() => {
          updateMemoryStatusDisplay(targetSlot, false); // Връща оригиналния фон след 3 секунди
      }, 300);
    ...
    case "0":
      Mem[targetSlot] = 0;
      console.log(`🧠 0 към Mem[${targetSlot}] → +${value} = [${Mem}]`);
      break;
    default:
...
/*
 * memoryShow: Временно показва стойността от даден слот на паметта
 * в горния дисплей, без да променя текущото изчисление.
 * Приема:
 *   - slot: Номер на слота на паметта (1, 2, 3).
 */
...
function updateStatus(message, sArea) {
  ...
  // Анимационен ефект: показване с трансформация
