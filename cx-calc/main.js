    ...
    // Това създава единствен източник за стойностите по подразбиране.
    ...
    function initializeCalculatorLayout() {
        try {
            // Изчисленията вече се случват след като сме сигурни, че изображението е заредено.
            getImageSize();
            getImageVisualSize();
            ...
            console.warn("⚠️ Грешка при инициализация на оформлението:", err);
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // 1. Дефинираме действието, което да се изпълни след зареждане на изображението.
        const onImageReady = () => {
            console.log("✅ Изображението е заредено. Извиквам initializeCalculatorLayout().");
            initializeCalculatorLayout();
        };

        // 2. Проверяваме дали изображението вече е заредено (напр. от кеша на браузъра).
        //    Ако е, изпълняваме функцията веднага, защото 'load' събитието може да не се задейства.
        if (calculator.complete) {
            onImageReady();
        } else {
            // 4. Ако не е, добавяме event listener, който ще изчака зареждането.
            //    Това е най-надеждният начин да се гарантира, че изчисленията ще са коректни.
            calculator.addEventListener('load', onImageReady);
            calculator.addEventListener('error', () => {
                console.error('Грешка при зареждане на фоновото изображение на калкулатора!');
                // Тук може да се покаже съобщение за грешка на потребителя.
            });
        }

        // --- ОСТАНАЛАТА ЛОГИКА ЗА ИНИЦИАЛИЗАЦИЯ ---
        loadSettings();
        ...
        // adjustFontSize и appendNumber("C") са преместени в initializeCalculatorLayout,
        // за да се изпълнят след като оформлението е готово.
